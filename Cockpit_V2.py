...
class AnalysisThread(QtCore.QThread):
    ...
    def run(self):
        ...
        for i, src_path in enumerate(self.file_paths):
            ...
            # STATE UPDATE
            state_file = os.path.join(os.path.dirname(MATRIX_PATH), 'validation_state.json')
            state_data = {}
            ...
            for trk in filtered_new_tracks:
                ...
                if trk_id not in state_data[basename]:
                    state_data[basename][trk_id] = {
                        'KI': {
                            'lang': trk['Effektive Sprache (ISO-Code)'],
                            'sdh': trk['Schwerhörig-Schalter'],
                            'forced': trk['Anzeige erzwingen-Schalter'],
                            'name': ''
                        },
                        'Validated': {
                            'lang': False,
...
class Screen3Validator(QtWidgets.QWidget):
    ...
    def populate_track_list(self, movie_name):
        ...
        for i, row_idx in enumerate(self.auto_rows):
            ...
            if row['Name'] == movie_name:
                item = QtWidgets.QListWidgetItem(f"Spur {row['ID']} ({row['Art']})")
    ...
    def validate_field(self, field):
        film = self.current_row['Name']
        trk_id = str(self.current_row['ID'])
        state_file = os.path.join(os.path.dirname(MATRIX_PATH), 'validation_state.json')
        if os.path.exists(state_file):
            try:
                import json
